...
gantt
    ...
    section Phase 7 (Milestone 3): Intelligence & Multi-Channel
    Gemini Model Routing (Spec 014) :done, p7a, 2026-09-07, 1d
    Vector Memory Engine (Spec 015) :done, p7b, after p7a, 1d
    Tool Execution Gateway (Spec 016) :done, p7c, after p7b, 1d
    Multi-Channel Gateway (Spec 017) :done, p7d, after p7c, 1d
    Disaster Recovery Restoration (Spec 018) :done, p7e, after p7d, 1d
    section Phase 8 (Milestone 4): Enterprise Ops & Autonomous Workflows
    Autonomous Cron Workflows (Spec 019) :done, p8a, 2026-09-07, 1d
    Multi-Tenant RBAC Authorization (Spec 020) :done, p8b, after p8a, 1d
    OpenTelemetry Distributed Tracing (Spec 021) :done, p8c, after p8b, 1d
    Dynamic Tool Plugin Sandbox (Spec 022) :done, p8d, after p8c, 1d
    HA Failover & Migration Verifier (Spec 023) :done, p8e, after p8d, 1d
    section Phase 9 (Milestone 5): Google Workspace via ClawHub
    Google Workspace ClawHub Skill (Spec 025) :p9a, after p8e, 2d
```
...
### Phase 6: Day-2 Operational Resilience & Security Hardening *(Complete)*
...
### Phase 7 (Milestone 3): Advanced Intelligence, Multi-Channel Gateway & Disaster Recovery *(Complete)*
* **Spec 014: Gemini Multi-Model Routing & Fallback Engine**: Multi-model dynamic switching (`gemini-2.5-flash` vs `gemini-2.5-pro`) via slash commands (`/model`) and automated HTTP 429 rate-limit fallback.
* **Spec 015: Vector Context Search & Semantic Memory Engine**: Persistent vector embedding storage on persistent disk (`vector_memory.db`) with Gemini embedding API, `/remember`, `/search`, and RAG context injection.
* **Spec 016: Tool Execution Gateway & Sandboxed Runtime**: Function calling engine for Gemini with restricted Python execution sandbox, persistent disk file operations (`/mnt/disks/openclaw-data`), 10s timeout bounds, and 2KB output truncation.
* **Spec 017: Multi-Channel Transport Gateway (Telegram & Discord)**: Abstract `ChannelAdapter` supporting Discord Bot API long-polling alongside Telegram with zero-trust access control whitelist parity.
* **Spec 018: Automated Disaster Recovery & Snapshot Restoration Verifier**: `scripts/disaster_recovery_restore.sh` utility to automate point-in-time state recovery from GCP disk snapshots with post-recovery SQLite database integrity checks.

### Phase 8 (Milestone 4): Enterprise Operations, Autonomous Workflows & Multi-Tenant Security *(Complete)*
* **Spec 019: Scheduled Autonomous Workflows & Cron Gateway**: Background cron scheduler engine for recurring prompts and automated tool executions with channel notifications and SQLite execution locks on persistent disk.
* **Spec 020: Multi-Tenant Authorization & RBAC Gateway**: Fine-grained role-based access control (`Admin`, `User`, `Read-Only`) for multi-tenant environments with dynamic role definition reloads.
* **Spec 021: OpenTelemetry APM & Distributed Tracing**: Complete observability instrumentation for Gemini API latency, vector search runtime, and tool execution tracing exported to GCP Cloud Trace.
* **Spec 022: Dynamic Tool Plugin Sandbox & Runtime**: Plugin architecture enabling dynamic loading of custom python/js extensions stored on persistent disk (`/mnt/disks/openclaw-data/plugins`).
* **Spec 023: High Availability Failover & Migration Verifier**: Automated zero-downtime failover and state synchronization verifier script (`scripts/verify_ha_failover.sh`) across regional standby nodes.

### Phase 9 (Milestone 5): Google Workspace Integration via ClawHub & gog *(Planned)*
* **Spec 025: Google Workspace ClawHub Skill (gog)**: Integrate Gmail, Google Calendar, Drive, Tasks, and Contacts into OpenClaw using the **ClawHub `gog` skill** framework rather than a custom MCP subprocess. The `gog` skill provides managed OAuth 2.0 authentication, automatic token refresh, and standardized Google API access that integrates natively with the Antigravity skill ecosystem.
  * Install the `clawhub/gog` skill to handle OAuth credential lifecycle.
  * Wire Gemini Function Calling in `tool_gateway.py` to dispatch Google Workspace tool calls via `gog` skill invocations.
  * No custom MCP subprocess, no manual `token.json` management.
  * Per **Principle 10** (updated): ClawHub skills are the **preferred** integration pattern for Google services; custom MCP servers are reserved only for non-Google third-party tools with no ClawHub equivalent.

> **Decision Record**: Spec 024 (Google Workspace MCP Server) was attempted but abandoned due to persistent JSON-RPC protocol compatibility issues between the `google-workspace-mcp` package and the Gemini Function Calling interface. The ClawHub `gog` approach is the approved replacement.
