...
def query_gemini(prompt, session_id="default"):
    ...
    payload = json.dumps({
        "system_instruction": {
            "parts": [{"text": "You are OpenClaw, an autonomous AI assistant with full active access to Google Workspace (Gmail, Calendar, Drive, Docs, Sheets, Tasks, Contacts) via Model Context Protocol (MCP) tools."}]
        },
...
def process_update(update):
    ...
    if text in ["/start", "/help"]:
        ...
        return
    elif text.startswith("/calendar") or "calendar" in text_lower or "schedule" in text_lower or "meeting" in text_lower:
        ...
        if events:
            ...
            send_telegram_message(chat_id, "📅 *Google Calendar MCP:* Direct tool execution active. (No upcoming events scheduled).")
        return
    elif text.startswith("/gmail") or "gmail" in text_lower or "email" in text_lower or "inbox" in text_lower:
        from tool_gateway import ToolGateway
        gw = ToolGateway()
        query = text[7:].strip() if text.startswith("/gmail") else "is:unread"
        ...
        if msgs:
            ...
            send_telegram_message(chat_id, f"📧 *Gmail MCP Integration Active:* Tool call dispatched via `uvx google-workspace-worker` for query '{query}'.")
        return
    elif text.startswith("/drive") or "drive" in text_lower or "doc" in text_lower or "sheet" in text_lower:
        from tool_gateway import ToolGateway
        gw = ToolGateway()
        query = text[6:].strip() if text.startswith("/drive") else "OpenClaw"
        ...
        if files:
            ...
            send_telegram_message(chat_id, f"📁 *Google Drive MCP Integration Active:* Tool call dispatched via `uvx google-workspace-worker` for query '{query}'.")
        return
