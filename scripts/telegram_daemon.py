...
def query_gemini(prompt):
    ...
    # Check for Google Calendar intent
    lower_p = prompt.lower()
    calendar_context = ""
    if any(kw in lower_p for kw in ["calendar", "schedule", "event", "meeting", "agenda"]):
        try:
            import calendar_tool
            cal_res = calendar_tool.get_calendar_events()
            if cal_res.get("status") == "success":
                events = cal_res.get("events", [])
                if events:
                    event_lines = [f"- {e['summary']} at {e['start']}" for e in events]
                    calendar_context = f"\n[System Info: User's upcoming Google Calendar events:\n" + "\n".join(event_lines) + "]\n"
                else:
                    calendar_context = "\n[System Info: User has no upcoming events on Google Calendar.]\n"
            else:
                return f"📅 Google Calendar Integration Notice:\n{cal_res.get('message')}"
        except Exception as e:
            print(f"[DAEMON] Calendar tool exception: {e}", flush=True)

    model = "gemini-2.5-flash"
    url = f"https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent?key={api_key}"
    full_prompt = prompt + calendar_context if calendar_context else prompt
